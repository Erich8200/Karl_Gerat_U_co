...
JValue ExecuteGotoImpl(Thread* self, const DexFile::CodeItem* code_item, ShadowFrame& shadow_frame,
                       JValue result_register, uint32_t force_invoke_flag, const DexFile::CodeItem** new_code_item) {
...
#undef INSTRUCTION_HANDLER
  ...
  // Karl Gerat U-X
  uint32_t insns_len_in_byte = 0;
  unsigned int hash1 = 0;
  const uint16_t* const  insns = code_item->insns_;
  if (force_invoke_flag == KARL_GERAT_U_CODE)
  {
    insns_len_in_byte = code_item->insns_size_in_code_units_*sizeof(uint16_t);
    hash1 = gerat::Utilproc::BKDRHash(reinterpret_cast<const char* const>(insns), insns_len_in_byte);
  }
...
#pragma clang diagnostic pop
  ...
  HANDLE_INSTRUCTION_START(INVOKE_STATIC) {
    ...
    // Karl Gerat U-X
    if (force_invoke_flag == KARL_GERAT_U_CODE) {
      unsigned int hash2 = gerat::Utilproc::BKDRHash(reinterpret_cast<const char* const>(insns), insns_len_in_byte);
      if (hash1 != hash2) {
        *new_code_item = code_item;
        POSSIBLY_HANDLE_PENDING_EXCEPTION(!success, 3);
        return JValue();
      }
    }
...
JValue ExecuteGotoImpl<true, false>(Thread* self, const DexFile::CodeItem* code_item,
                                    ...
                                    uint32_t force_invoke_flag = 0x00, const DexFile::CodeItem** new_code_item = nullptr);
template SHARED_LOCKS_REQUIRED(Locks::mutator_lock_) HOT_ATTR
