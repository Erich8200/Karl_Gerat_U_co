...
#include "gerat/unpack/unpack_globals.h"
#include "gerat/util/Defs.h"
...
class ArtMethod FINAL {
 public:
  ...
  // Karl Gerat U
  void check_and_repair_DEX_magic(uint8_t* dex_begin);
  void write_file(uint8_t* dex_file_m, size_t dex_size_real);
  void dump_dex();
  void dump_dex_after_init(Thread* self);
  void build_dex(Thread* self);
  long get_codeitem_off_by_iter(art::ClassDataItemIterator& class_data_it, const unsigned char* begin_);
 ...
 private:
  ...
  {
  private:
    std::map<uint32_t,uint32_t>                       code_item_offset_map;
    std::map<uint16_t,uint32_t>                       class_data_offset_map;
    uint8_t*                                          code_item_seg;
    uint8_t*                                          class_data_item_seg;
    uint8_t*                                          cur_code_item_offset;
    uint8_t*                                          cur_class_data_offset;
    uint8_t*                                          class_data_addr;
    uint8_t*                                          code_item_addr;
    std::vector<gerat::ClassDataItemMem>              class_data_items;
    uint8_t*                                          begin; // Begin of the DEX
    size_t                                            dex_size;

  public:  
    void init_segments(const DexFile* dex_file);
    void clear_segments();
    uint32_t get_dex_max_seg_size(const DexFile* dex_file);
    void build_ClassDataItem(mirror::Class* klass);
    void build_CodeItem(ArtMethod* method);
    void build_CodeItem(ArtMethod* method, mirror::Class* klass);
    void build_ClassDataItem2(Thread* self, ArtMethod* refering_method, const DexFile* dex_file, const DexFile::ClassDef& class_def); // Build from ClassDef, statically
    uint8_t* copy_DEX_data(const DexFile* dex_file, uint8_t* dex_file_m_); // Second param is not DexFile
    uint8_t* copy_code_items(uint8_t* target);
    uint8_t* copy_class_data_items(uint8_t* target);
    void repair_Header(const DexFile* dex_file, uint8_t* dex_file_m_); // not DexFile
    void repair_ClassDefs(const DexFile* dex_file, uint8_t* dex_file_m_);
    void repair_Magic(uint8_t* dex_file_m_);
    uint8_t* codeitem_end(const uint8_t ** pData);
    void set_dex_size(size_t size);
    const DexFile::CodeItem* get_code_item_by_force_invoke(ArtMethod* method, mirror::Class* klass);
    std::unordered_set<uint16_t>                      built_class_idx_set;
  };
